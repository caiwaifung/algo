...
/*
const int MAX_OPERATIONS = 200010;
const int MAX_JEWELS_START_LEN = 200010;
const int MAX_JEWELS_LEN = MAX_JEWELS_START_LEN + MAX_OPERATIONS;
*/
...
public:
    ...
    /*
    int64 merge(int64 key, int64 keyVal, int keyLen) {
        return (key * powerMod[keyLen] + keyVal) % modNumber;
    }*/
} hashManager;
...
class Splay {
    ...
    SplayNode *splay(SplayNode *root, int index) {
        vector<SplayNode*> path;
        for (SplayNode *node = root; ; ) {
            path.push_back(node);
            node -> clearReversion();
            int sz = node->lChild->size;
            if (index == sz+1)
                break;
            if (index <= sz)
                node = node->lChild;
            else
                node = node->rChild, index -= sz+1;
        }
        while (path.size() > 1) {
            SplayNode *nodeA, *nodeB, *nodeC;
            nodeA = path.back(), path.pop_back();
            nodeB = path.back(), path.pop_back();
            if (path.size() > 0) {
                nodeC = path.back(), path.pop_back();
                
                if (nodeA == nodeB->lChild && nodeB == nodeC->lChild) {
                    nodeC->lChild = lRotate(nodeB);
                    path.push
                } else
            } else {
                if (nodeA == nodeB->lChild)
                    path.push_back(lRotate(nodeB));
                else
                    path.push_back(rRotate(nodeB));
            }
        }
        return path[0];
    }
