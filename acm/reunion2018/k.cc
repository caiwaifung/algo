...
int solve(LL a, LL b, LL c, LL mean) {
    if (a > b || b > c) return -1;
    const auto check = [&](int n) {
        if (n == 1) return (a == b && b == c && mean == a);
        if (n == 2) return (mean == b && a + c == b + b);
        LL sum = n * mean;
        if (n % 2 == 1) {
            LL p = (n - 3) / 2;
            LL l = a + b + c + p * a + p * b;
            ...
            LL p = (n - 4) / 2;
            LL l = a + b * 2 + c + p * a + p * b;
            LL r = a + b * 2 + c + p * b + p * c;
            return l <= sum && sum <= r;
        }
    };
    rep(n, 1, 1000000) {
...
int main() {
    for (int num = IN, cs = 1; cs <= num; ++cs) {
        LL a = IN, c = IN, mean = IN, b = IN;
        int ans = solve(a, b, c, mean);
        printf("Case #%d: ", cs);
        if (ans >= 0) {
            printf("%d\n", ans);
        } else {
