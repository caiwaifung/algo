...
double median(VD a, VD w) {
    double sw = accumulate(all(w), 0.0);
    double cur = 0;
    vector<PDD> r;
    repn(i, len(a)) r.pb(mp(a[i], w[i]));
    sort(all(r));
    for (const auto& p : r) {
        cur += p.se;
        if (cur * 2 >= sw) return p.fi;
    }
    return 1e50;
}
...
double solve() {
    ...
    double mu = median(u, w), mv = median(v, w);
    double ans = 0;
    repn(i, n) { ans += w[i] * (fabs(u[i] - mu) + fabs(v[i] - mv)); }
    return ans;
}
