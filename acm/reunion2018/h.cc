...
VPI simplify(VPI a) {
    VI xs, ys;
    for (const auto& p : a) {
        xs.pb(p.fi), ys.pb(p.se);
    }
    sort(all(xs)), xs.erase(unique(all(xs)), xs.end());
    sort(all(ys)), ys.erase(unique(all(ys)), ys.end());
    for (auto& p : a) {
        p.fi = int(lower_bound(all(xs), p.fi) - xs.begin()) + 1;
        p.se = int(lower_bound(all(ys), p.se) - ys.begin()) + 1;
    }
    return a;
}
...
bool solve(VPI ps) {
    ...
    vector<bitset<4040>> a(A + 2), a1(A + 2), a2(A + 2);
    for (const auto& p : ps) {
        a[p.fi][p.se] = 1;
    }
    rep(i, 0, A) rep(j, 0, B) {
        if (a[i][j]) a1[i][j] = a2[i][j] = 1;
        if (a1[i][j]) a1[i + 1][j] = 1;
        if (a2[i][j]) a2[i][j + 1] = 1;
    }

    vector<bitset<4040>> f(A + 2), g(A + 2), h1(A + 2), h2(A + 2);
    irep(i, A, 0) irep(j, B, 0) {
        f[i][j] = (g[i + 1][j + 1] || h1[i + 1][j] || h2[i][j + 1]);
        if (!f[i][j]) {
            if (a[i][j]) g[i][j] = 1;
            if (a2[i][j]) h1[i][j] = 1;
            if (a1[i][j]) h2[i][j] = 1;
        ...
        if (h1[i][j] || h1[i + 1][j]) h1[i][j] = 1;
        if (h2[i][j + 1]) h2[i][j] = 1;
    }
    return f[0][0];
}
