class Delaunay{
    ...
    class Point3D {
        ...
        Point3D det(Point3D p) {
            return new Point3D(y * p.z - z * p.y, z * p.x - x * p.z,
            		x * p.y - y * p.x);
        }
    ...
    void construct(int l, int r) {//[l,r)
        ...
        for (; ; ) {
            ...
            for (PointId p : pl.edges) {
                ...
                if (op > 0 || 
                		(op == 0 && p.distTo2(pr) < pl.distTo2(pr))) {
                    next = p;
                    break;
                }
            }
            if (next != null)
                ...
                for (PointId p : pr.edges) {
                    int op = crossOp(pr, pl, p);
                    if (op > 0 || 
                    		(op == 0 && p.distTo2(pl) < pr.distTo2(pl))) {
                        next = p;
                        break;
                    }
                }
        ...
        for (; ; ) {
            ...
            boolean which = false;
            for (PointId p : pl.edges) {
                if (crossOp(pr, pl, p) < 0 && 
                (next == null || inCircle(next, pl, pr, p) == -1))
                    next = p;
            }
            for (PointId p : pr.edges) {
                if (crossOp(pl, pr, p) > 0 && 
                (next == null || inCircle(next, pl, pr, p) == -1)) {
                    next = p;
                    which = true;
                }
