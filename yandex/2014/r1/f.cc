...
// 20:55 - 21:18
// 21:18 - eta 21:25
//       - 21:23 - wa - missed a case
//       - 21:32 - wa - wrong in solve1
//       - 21:39 - wa
//       - 22:43 - pass
#include <algorithm>
#include <cstdio>
...
void solve2(VL a, int k) {
    ...
    repn(i, n - 1) {
        LL u = min(a[i], a[i + 1]);
        LL v = max(a[i], a[i + 1]);
        if(setmax(ans2, xp * u + yp * v)) ans = (x * u + y * v) % MOD;
    }
    printf("%lld\n", ans);
}

void solve1(VL a, int k) {
    sort(all(a));
    printf("%lld\n", a[k]);
}
