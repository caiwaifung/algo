...
int vio(VVI es) {
    ...
    const function<int(VI)> f = [&](VI a) {
        ...
        if(!found) {
            VI ff(n, -1);
            const function<int(int)> gg = [&](int i) {
                return ff[i] < 0 ? i : ff[i] = gg(ff[i]);
            };
            repn(x, n) for(int y : es[x]) {
                if(a[x] == a[y]) ff[gg(x)] = y;
            }
            ans = 0;
            repn(x, n) if(ff[x] < 0) ans -= a[x];
        }
        return ans;
    };
    return f(VI(n, 0));
...
int main() {
    while(1) {
        int n = rand() % 5 + 5;
        VVI es(n);
        replr(i, 1, n) es[i].pb(rand() % i);
        // repn(i, n - 1) es[i].pb(i + 1);
        cout << vio(es) << " " << es << endl;
    }
    int n;
    scanf("%d", &n);
