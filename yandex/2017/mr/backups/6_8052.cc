...
void adjust2() {
    repn(_, 200) {
        ...
        repn(color, k) {
            ...
            VPI path = find_path(assignment, o1, o2, color);
            assert(!path.empty());
        ...
        for(const auto& u1 : allpos) {
            for(const auto& u2 : neighbors[u1.x][u1.y]) {
                if(occupied[u1.x][u1.y] >= 0 || occupied[u2.x][u2.y] >= 0 ||
                   assignment[u1.x][u1.y] != assignment[u2.x][u2.y])
                    continue;
                unordered_set<int> colors;
                for(const auto& v : neighbors[u1.x][u1.y]) {
                    colors.insert(assignment[v.x][v.y]);
                }
                for(const auto& v : neighbors[u2.x][u2.y]) {
                    colors.insert(assignment[v.x][v.y]);
                }
                colors.erase(assignment[u1.x][u1.y]);
                for(int c2 : colors) {
                    int c1 = assignment[u1.x][u1.y];
                    int e1 = 6, e2 = 6;
                    for(const auto& v : neighbors[u1.x][u1.y]) {
                        if(v == u2) continue;
                        int c = assignment[v.x][v.y];
                    ...
                    double old_score = score(cells[c1], edges[c1]) +
                                       score(cells[c2], edges[c2]);
                    double new_score = score(cells[c1] - 2, edges[c1] - e1) +
                                       score(cells[c2] + 2, edges[c2] + e2);
                    if(old_score < new_score) {
                        assignment[u1.x][u1.y] = c2;
                        assignment[u2.x][u2.y] = c2;
                        cells[c1] -= 2, edges[c1] -= e1;
                        cells[c2] += 2, edges[c2] += e2;
                        found = true;
                    }
                }
            }
