...
public:
    ...
    bool operator==(const Hash& b) const {
        return h1 == b.h1 && h2 == b.h2 && size == b.size;
    }
...
int main() {
    ...
    unordered_set<Hash> cs;
    for(const auto& s : str) {
        Hash h;
        for(char c : s) h = h + Hash(c);
        cs.insert(h);
    }
    unordered_map<Hash, int> cnt;
    for(auto& s : str) {
        Hash h0;
        for(char c : s) h0 = h0 + Hash(c);
        Hash h;
        s.pop_back();
        for(char c : s) {
            h = h + Hash(c);
            if(cs.count(h)) cnt[h0.remove_l(h)]++;
        }
    }
    LL ans = 0;
    for(const auto& kv : cnt) ans += LL(kv.se) * LL(kv.se - 1) / 2;
    cout << ans << endl;
    return 0;
}
