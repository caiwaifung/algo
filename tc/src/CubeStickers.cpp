...
public:
	string isPossible(vector <string> sticker) {
		map<string, int> m;
		forn(i, sticker.size()) m[sticker[i]]++;
		VI a;
		for(map<string,int>::iterator it=m.begin(); it!=m.end(); ++it)
			a.PB(it->se);
		if(a.size()>=6)
			return "YES";
		forn(w, a.size()) forn(x, a.size()) forn(y, a.size()) forn(z, a.size()) {
			if(w==x || x==y || y==z || z==w) continue;
			VI b=a;
			b[w]--; b[x]--; b[y]--; b[z]--;
			bool flag=true;
			forn(i, b.size()) if(b[i]<0) flag=false;
			if(!flag) continue;
			b[w]=0, b[x]=0, b[y]=0, b[z]=0;
			int s=0;
			forn(i, b.size()) s+=b[i];
			if(s>=2) return "YES";
		}
