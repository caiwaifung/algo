...
public:
    vector <int> construct(vector <int> d) {
        ...
        int mind=1<<30, minc=0, maxd;
        repn(i, n) {
            if(setmin(mind, d[i])) minc=0;
            if(d[i]==mind) ++minc;
        }
        VI cd;
        if(minc==1) {
            irep(v, mind*2, mind) cd.pb(v);
            rep(v, mind+1, mind*2) cd.pb(v);
            maxd=mind*2;
        } else {
            irep(v, mind*2-1, mind) cd.pb(v);
            rep(v, mind, mind*2-1) cd.pb(v);
            maxd=mind*2-1;
        }
        ...
        VI ans;
        replr(k, 1, sz(ci)) ans.pb(ci[k-1]), ans.pb(ci[k]);
        repn(i, n) if(!used[i]) {
            if(d[i]<=mind || d[i]>maxd) return VI();
            ans.pb(ci[maxd-d[i]+1]), ans.pb(i);
        }
        return ans;
    }
...
int main(){
    ...
    cout<<___test.construct({1,1})<<endl;
