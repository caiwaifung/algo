...
int solve(int N, int K, int MOD) {
    static int f[303][303][303];
    const auto add = [MOD](int& a, LL b) { a = int((LL(a) + b) % MOD); };
    fillchar(f, 0);
    f[0][0][0] = 1;
    repn(k, K) irep(i, N, 0) rep(n, i, N) {
        const LL cur = f[k][n][i];
        // if(cur) printf("%d %d %d -> %d\n",k,n,i,cur);
        add(f[k][n + 1][i], cur * (i + 1));
        if (i > 0) {
            add(f[k][n][i - 1], cur);
        } else {
            add(f[k + 1][n][n], cur);
