...
int main() {
    ...
    rep(i, 0, m) repn(s, 1<<n) f[i][s].clear();
    f[0][0].pb(State(0, -1, -1));
    rep(i, 0, m-1) {
        //printf("i=%d\n",i);
        for(int s2: cols[b[i]])
            repn(s, 1<<n) {
                for(const State &cur: f[i][s]) {
                    ...
                    f[i+1][s2].pb(State(t2, s, t));
                }
            sort(all(f[i+1][s2]));
            f[i+1][s2].erase(unique(all(f[i+1][s2])), f[i+1][s2].end());
        }
    }
    /*
    rep(i, 0, m) {
    ...
    */
    ...
    int s0=-1, t0=-1;
    repn(s, 1<<n) for(const State &cur: f[m][s]) {
        int t=cur.t;
    ...
    irepn(i, m) {
        ...
        //printf("\n");
        State state=*lower_bound(all(f[i+1][s0]), State(t0,0,0));
        s0=state.pre_s;
        t0=state.pre_t;
    }
    repn(i, n) {
