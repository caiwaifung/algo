...
int up[N][LOG], jump[N][LOG];
LL upm[N][LOG], jumps[N][LOG];
int n;

void add(int x, LL w) {
    int y = n++;
    up[y][0] = x, upm[y][0] = w;
    for (int i = 0; up[y][i] >= 0; ++i) {
        up[y][i + 1] = up[up[y][i]][i];
        upm[y][i + 1] = max(upm[y][i], upm[up[y][i]][i]);
    }

    irepn(i, LOG) {
        if (up[x][i] >= 0 && upm[x][i] < w) {
            x = up[x][i];
        }
    }
    if (upm[x][0] < w) {
    ...
    jump[y][0] = x, jumps[y][0] = w;
    for (int i = 0; jump[y][i] >= 0; ++i) {
        jump[y][i + 1] = jump[jump[y][i]][i];
        jumps[y][i + 1] = jumps[y][i] + jumps[jump[y][i]][i];
    }
}

LL query(int x, LL limit) {
    int ans = 0;
    irepn(i, LOG) {
        if (jump[x][i] >= 0 && jumps[x][i] <= limit) {
            limit -= jumps[x][i];
            x = jump[x][i];
            ans += 1 << i;
        }
...
int main() {
    ...
    for (int qs = IN; qs--;) {
        int t = IN;
        LL p = read_ll(), q = read_ll();
        if (t == 1) {
            add(int(p ^ last) - 1, q ^ last);
        } else {
            LL ans = query(int(p ^ last) - 1, q ^ last);
            printf("%lld\n", ans);
            last = ans;
        }
    }
