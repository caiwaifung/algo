...
int main() {
    const VPI a = read_vpi();
    static int f[2001][10][10][10];
    fillchar(f, -1);
    const function<int(int, VI)> cal = [&](int i, VI ts) {
        if (len(ts) > 3) return 1 << 30;
        sort(all(ts));
        const int t0 = len(ts) > 0 ? ts[0] : 0;
        const int t1 = len(ts) > 1 ? ts[1] : 0;
        const int t2 = len(ts) > 2 ? ts[2] : 0;
        int& ans = f[i][t0][t1][t2];
        if (ans >= 0) return ans;
        if (i == len(a)) {
            return ans = (len(ts) == 0 ? 0 : 1 << 30);
        }
        ts.pb(a[i].se), sort(all(ts));
        ans = 1 << 30;
        do {
            int cur = a[i].fi, steps = 0;
            rep(k, 0, len(ts)) {
                int tmp = cal(i + 1, VI(ts.begin() + k, ts.end()));
                tmp += steps;
                if (i + 1 < len(a)) {
                    tmp += abs(cur - a[i + 1].fi);
                }
                setmin(ans, tmp);
                if (k < len(ts)) {
                    steps += abs(cur - ts[k]);
        ...
        } while (next_permutation(all(ts)));
        return ans;
    };
    int ans = cal(0, {}) + len(a) * 2 + a[0].fi - 1;
    printf("%d\n", ans);
    return 0;
}
