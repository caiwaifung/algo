...
int main() {
    ...
    cin >> a >> b;
    int n = sz(a);
    sort(all(a)), sort(all(b), greater<char>());
    ...
    while(i < sz(a) && j < sz(b)) {
        if(a[i] < b[j]) {
            ...
            suffix += a.back(), a.pop_back();
        }
        if(a[i] < b[j]) {
            prefix += b[j++];
        } else {
            suffix += b.back(), b.pop_back();
        }
    }
    if(i < sz(a)) prefix += a[i];
    while(!suffix.empty()) prefix += suffix.back(), suffix.pop_back();
    cout << prefix << endl;
    return 0;
}
