...
int main() {
    ...
    LL base = 1ll << 60;
    repn(i, n) repn(j, n) if(i != j) setmin(base, g[i][j]);
    repn(i, n) repn(j, n) if(i != j) g[i][j] -= base;
    VL m(n, 1ll << 60);
    repn(i, n) repn(j, n) if(i != j) setmin(m[i], g[i][j]);
    ...
    repn(i, n) if(dis[i] == 0) {
        repn(j, n) if(setmin(dis[j], 2 * m[j])) {
            if(!inside[j]) {
                inside[j] = true;
                que.push(j);
            }
