...
int main() {
    ...
    cin >> s >> t;
    const auto ok = [&](int i) {
        repn(j, sz(t)) if(s[i + j] != '?' && s[i + j] != t[j]) return false;
        return true;
    };
    VI fail(sz(t) + 1);
    {
    ...
    VB good(sz(t) + 1, false);
    for(int i = sz(t); i >= 0; i = fail[i]) good[sz(t) - i] = true;

    VI f(sz(t), 0);
    repn(i, sz(s) - sz(t) + 1) {
        VI f2(sz(t), 0);
        repn(j, sz(t)) setmax(f2[min(j + 1, sz(t) - 1)], f[j]);
        if(ok(i)) {
            repn(j, sz(t)) if(good[j + 1]) setmax(f2[0], f[j] + 1);
        }
        f = f2;
    }
