...
int main() {
    int n = IN, m = IN, ns = IN, ne = IN, v = IN;
    (void)n, (void)m;
    VI ss = read_vi(ns);
    VI es = read_vi(ne);
    for (int cs = IN; cs > 0; --cs) {
        ...
        int ans = 1 << 30;
        repn(op, 2) {
            const VI& ps = (op == 0 ? ss : es);
            const int pv = (op == 0 ? 1 : v);
            const int xtime = (abs(x1 - x2) + pv - 1) / pv;
            if (ps.empty()) continue;
            auto it = lower_bound(all(ps), y1);
            if (it != ps.end() && *it <= y2) {
                setmin(ans, xtime + abs(y1 - y2));
            }
            if (it != ps.begin()) {
                --it;
                setmin(ans, xtime + abs(y1 - *it) + abs(y2 - *it));
            }
            it = lower_bound(all(ps), y2);
            if (it != ps.end()) {
                setmin(ans, xtime + abs(y1 - *it) + abs(y2 - *it));
            }
        }
