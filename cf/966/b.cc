...
pair<VI, VI> solve(VPI a, int x1, int x2) {
    int cur = 0;
    VVI ans;
    for (int x : {x1, x2}) {
        int cnt = 1;
...
int main() {
    const int n = IN, x1 = IN, x2 = IN;
    VPI a(n);
    repn(i, n) a[i].fi = IN, a[i].se = i;
    sort(all(a), std::greater<PII>());
    pair<VI, VI> ans = solve(a, x1, x2);
    if (ans.fi.empty()) {
        ans = solve(a, x2, x1);
        swap(ans.fi, ans.se);
    }
    if (ans.fi.empty()) {
        ...
        printf("Yes\n");
        printf("%d %d\n", len(ans.fi), len(ans.se));
        for (int i : ans.fi) printf("%d ", i + 1);
        printf("\n");
