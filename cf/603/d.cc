...
int main() {
    ...
    LL ans = 0;
    int pass = 0;
    repn(i, n) if(c[i] == 0)++ pass;
    assert(pass <= 2);
    if(pass == 2) ans += n - 2;

    repn(i, n) if(c[i]) {
        vector<PLL> ds;
        replr(j, i + 1, n) if(c[j]) {
            LL d1 = a[i] * a[i] + b[i] * b[i];
            LL d2 = a[j] * a[j] + b[j] * b[j];
            LL x = c[j] * a[j] * d1 - c[i] * a[i] * d2;
            LL y = c[j] * b[j] * d1 - c[i] * b[i] * d2;
            assert(x || y);
            LL g = gcd(x, y);
            x /= g, y /= g;
        ...
        sort(all(ds));
        for(int l = 0, r; l < sz(ds); l = r) {
            for(r = l; r < sz(ds) && ds[l] == ds[r];) ++r;
            ans += (r - l) * (r - l - 1) / 2;
        }
    }
    cout << ans << endl;
