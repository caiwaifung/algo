...
int main() {
    int n = IN, a = IN, b = IN, b0 = b;
    VPL d = read_vpl(n);
    sort(all(d),
         [](const PLL& u, const PLL& v) { return u.fi - u.se > v.fi - v.se; });
    setmin(b, n);
    while (b > 0 && d[b - 1].fi < d[b - 1].se) --b;
    LL ans0 = 0;
    repn(i, b) ans0 += d[i].fi;
    replr(i, b, n) ans0 += d[i].se;
    LL ans = ans0;
    if (b0 > 0) {
        repn(i, n) {
            LL cur = ans0;
