...
int main() {
    ...
    repn(i, k) {
        int x, p;
        scanf("%d%d", &x, &p), --x, --p;
        if(where[p] >= 0) in[x] = where[p];
        where[p] = x;
    }
    VVI es(n);
    repn(i, n) if(in[i] >= 0) es[in[i]].pb(i);
    VI l(n), r(n), d(n);
    int cur = 0;
    const function<void(int, int)> dfs = [&](int x, int dep) {
        l[x] = cur++;
        d[x] = dep;
        for(int y : es[x]) dfs(y, dep + 1);
        r[x] = cur;
    };
    repn(i, n) if(in[i] < 0) dfs(i, 0);
