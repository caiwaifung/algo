...
int main() {
    ...
    for(auto& e : es) {
        if(e.inside) {
            tie(e.a, e.b) = mp(0, cur);
            replr(i, 1, cur) {
                if(sz(candidates) > m) break;
                candidates.pb({i, cur});
            }
            ++cur;
        } else {
            ...
            tie(e.a, e.b) = candidates.back();
            candidates.pop_back();
        }
    }
    sort(all(es), [](const Edge& a, const Edge& b) { return a.i < b.i; });
    for(const auto& e : es) {
        printf("%d %d\n", e.a + 1, e.b + 1);
    }
