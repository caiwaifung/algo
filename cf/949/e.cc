...
VI solve(VI xs, int cur) {
    if (xs.empty()) return {};
    bool found = false;
    if (cur > (200000)) return VI(22);
    for (int x : xs) {
        if (x & cur) found = true;
    }
    if (!found) {
        return solve(xs, cur * 2);
    }

    VI ans;
    for (int choose : {-cur, cur}) {
        VI ys;
        ++id;
        for (int x : xs) {
            if (x & cur) x -= choose;
            if (x != 0 && used[x + 300000] < id) {
                used[x + 300000] = id;
                ys.pb(x);
            }
        }
        VI tmp = solve(ys, cur * 2);
        tmp.pb(choose);
        if (ans.empty() || len(tmp) < len(ans)) ans = tmp;
    }
    return ans;
}
...
int main() {
    fillchar(used, 0);
    VI xs = read_vi();
    xs.erase(remove(all(xs), 0), xs.end());
    VI ans = solve(xs, 1);
    printf("%d\n", len(ans));
