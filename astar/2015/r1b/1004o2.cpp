...
bool check(int i, LL a) {
    ...
    if(r>=tlen+1) {
        ...
        if(maxa*x<an*p10[n-1]*times) return false;
        LL mina=a1*p10[n-1];
        if(mina*x>(an*p10[n-1]+p10[n-1])*times) return false;
    }
    if(r>=tlen+2) {
...
void solve(int cs) {
    ...
    sort(all(ans));
    if(ans.size()>0) {
        //forn(i, ans.size()) printf("%I64d ", ans[i]);
        forn(i, ans.size()) printf("%lld ", ans[i]);
        printf("\n");
    }
}
